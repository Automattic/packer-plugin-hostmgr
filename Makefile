...
publish: validate-version
	docker run -v $(shell pwd):/app -w /app -e BUILDKITE_TAG=$(VERSION) -e GITHUB_TOKEN ruby:3.3.4 /bin/bash -c "bundle install && bundle exec ruby .buildkite/upload-artifacts.rb"
